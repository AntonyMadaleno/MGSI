...
int main() {
    ...
    glfwSwapInterval(1); // Enable V-Sync (limits frame rate to the refresh rate)

    // Create a GLFW window
    GLFWwindow* window = glfwCreateWindow(1600, 900, "Curve Test", NULL, NULL);
    if (!window) {
    ...
    lights_Positions[0] = glm::vec3( 5.0f,  5.0f,  5.0f);
    ...
    //LOAD SKYBOX
    const char* fpath = "../textures/skybox/skybox.bmp";
...
/**
    ...
    Curve3D * curve = Curve3D_init(control_points, 11, 2000, CATMULL_ROM);
    ...
    Object * obj = Curve3D_generateSurface(curve, 32, obj_color, obj_specular_color, 1.0f, 0.25f, 0.25f);
...
*/
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Set the model matrix to rotate the cube
        glm::mat4 u_model(1.0f);
        float angle = glm::radians((float)glfwGetTime() * 12.0f); // Rotate over time
        shader.setVec3_array("u_lights_Positions", lights_Positions[0], light_count);
        cameraPosition = glm::vec3(10.0f * cosf(angle), 0.0f, 10.0f * sinf(angle));     // Position the camera at
        glm::vec3 cameraTarget = glm::vec3(0.0f, 0.0f, 0.0f);       // Look at the origin
        glm::vec3 cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);           // Set the up vector
        u_view = glm::lookAt(cameraPosition, cameraTarget, cameraUp);
        shader.setMat4("u_view", u_view, GL_FALSE);
        ...
        // Draw the cube using indices (element buffer)

        Buffer_bind(buffer_0);
        glDrawElements(GL_TRIANGLES, obj->n_faces * 3, GL_UNSIGNED_INT, 0);
